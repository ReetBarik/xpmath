// Kokkos TF (triple-float) complex ops demo -- TIMING AND SMOKE ONLY.
// Each operation is run on Kokkos::Experimental::TripleFloatComplex and on
...
static void print_complex_op_rows(const ComplexOpResult& r) {
  ...
  const TimeStats& t1 = r.tf_timing;
...
int main(int argc, char** argv) {
  ...
  Kokkos::initialize(argc, argv);
  {
    std::cout << "\nbatch=" << cfg.batch << "  repeats=" << cfg.repeats
              ...
              << "  timing=kernel+fence\n\n";
    print_header("Kokkos TF (triple-float)", "CUDA FP64");
