// Kokkos float-float demo — real ops. TIMING AND SMOKE ONLY.
// Times Kokkos::Experimental::FloatFloat against FP64, op by op.
//
...
//
// Construction
//   ff::FloatFloat x;               // zero
//   ff::FloatFloat x(1.5f);         // from float
//   ff::FloatFloat x(hi, lo);       // from two float components
//
// Arithmetic operators
//   x + y,  x - y,  x * y,  x / y    // FloatFloat op FloatFloat -> FloatFloat
//   x + b,  x - b,  x * b,  x / b    // FloatFloat op float  -> FloatFloat
//   a + y,  a - y,  a * y,  a / y    // float  op FloatFloat -> FloatFloat
//   -x                                // unary negation
//   x += y, x -= y, x *= y, x /= y   // FloatFloat op FloatFloat
//   x += b, x -= b, x *= b, x /= b   // FloatFloat op float
//
...
//   ff::pow(x, y)                // FloatFloat exponent
//   ff::pow_int(x, n)            // int exponent
//   ff::hypot(x, y)
//   ff::erf(x),   ff::erfc(x)
//   ff::tgamma(x)
...
struct OpResult { Op op; TimeStats ff_timing, dbl_timing; };
...
OpResult run_op(Op op, const Config& cfg) {
  ...
  vdbl ad("ad",n), bd("bd",n), cd("cd",n), rd("rd",n);
  {
    ...
    for (int i=0; i<n; ++i) {
      ...
      maff(i)=ff::FloatFloat(ha[i]); mbff(i)=ff::FloatFloat(hb[i]);
      mcff(i)=ff::FloatFloat(hc[i]);
