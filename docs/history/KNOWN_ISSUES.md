> **HISTORICAL — this is not a list of open issues.**
>
> Every entry below was found, diagnosed and (with the exceptions noted in each
> entry) fixed during development. It is kept for provenance: forty defects, what
> each one actually was, and what closing it involved. Several of the traps
> recorded here recur, which is the other reason to keep it.
>
> The **open** list is `validation/sweep/open_defects.txt`, enforced in both
> directions by the `sweep_absolute_gate` ctest target. How correctness is
> measured now is `docs/CORRECTNESS.md`.
