...
`docs/DOMAINS.md` is generated by `scripts/gen_domains.py` from the host
baseline, the grid and the register — per backend per op, the usable range and
where it degrades. It is guarded by the `domains_fresh` ctest target, so it
cannot drift from the data. Its opening section states that every number in it
is host-measured.

`docs/DEVICE_PRECISION.md` is generated by the same script with
`--device-precision`, reading the host, A100 and MI250 baselines. It answers the
four device mechanisms (FTZ/DAZ, FMA contraction, vendor libm, gfx90a
mitigations) with measured numbers and is guarded by `device_domains_fresh`.
An absent device baseline is emitted as an explicit `NO BASELINE` gap, never as
a missing column.
...
host.** One `cmake`, one compiler, all 65 registered tests including both gates
...
|---|---|---|
| `<dir>/host` | always `g++`, no Kokkos, identical on all three arches | `sweep_accuracy`, **both gates, both gate self-tests**, `sweep_device_gate_a100` / `sweep_device_gate_mi250` and the device-gate self-test, `oracle_conv_test`, `domains_fresh`, `device_domains_fresh` — 42 tests |
| `<dir>/device` | the arch's compiler and Kokkos | the `*_test_device` halves, the harness self-test, the demos — 24 tests |

42 + 24 = 66 and the union is 65: `build_provenance` is registered in every
...
python3 scripts/gen_domains.py --device-precision > docs/DEVICE_PRECISION.md
```
