...
host.** One `cmake`, one compiler, all 64 registered tests including both gates
...
|---|---|---|
| `<dir>/host` | always `g++`, no Kokkos, identical on all three arches | `sweep_accuracy`, **both gates, both gate self-tests**, `sweep_device_gate_a100` / `sweep_device_gate_mi250` and the device-gate self-test, `oracle_conv_test`, `domains_fresh` — 41 tests |
| `<dir>/device` | the arch's compiler and Kokkos | the `*_test_device` halves, the harness self-test, the demos — 24 tests |

41 + 24 = 65 and the union is 64: `build_provenance` is registered in every
