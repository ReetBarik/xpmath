...
- `cd56ac6` Phase 5 (this commit) — TF demos + consolidated STATUS
...
- **The TF demos were written without `copysign` or `hypot`** because TF does not implement them (they are in DD/FF/QF but not TF). The TF real demo has 38 operations (kAllOps[] count); QF/FF/DD have 39. This is a deliberate API subset, not an omission — tf_math.hpp does not forward-declare `copysign` or `hypot`.
