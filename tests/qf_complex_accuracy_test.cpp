// ============================================================================
// qf_complex_accuracy_test.cpp — per-op accuracy gate for QuadFloat COMPLEX
// ============================================================================
//
...
//         tests/qf_complex_accuracy_test.cpp -o /tmp/t && /tmp/t
//
// WHAT IS SCORED
//
// The 24 complex operations shared by all four backends, re-derived from
// include/xp/qf_complex.hpp: the four operators (add sub mul div) plus the 20
...
// convention of src/demo_qf_complex.cpp) as
//
...
// ---------------------------------------------------------------------------
using Scalar  = xp::QuadFloat;
using Complex = xp::QuadFloatComplex;

// 4 x FP32 -> ~29 decimal digits. Same cap as qf_accuracy_test / demo_qf_complex.
static constexpr double kMaxDig = 29.0;

static constexpr const char* kBackend = "QF (QuadFloat, 4 x FP32)";

// A corpus input is a `double`. QuadFloat(double) splits it across four FP32 limbs
// (96 bits >= 53), so the split is exact and QF is scored on precisely the value
// the oracle saw. FF is not — see the note in ff_complex_accuracy_test.cpp.
...
static Got words(Complex z) {
  Got g;
  g.nre = 4; g.re[0] = double(z.re.f0); g.re[1] = double(z.re.f1);
             g.re[2] = double(z.re.f2); g.re[3] = double(z.re.f3);
  g.nim = 4; g.im[0] = double(z.im.f0); g.im[1] = double(z.im.f1);
             g.im[2] = double(z.im.f2); g.im[3] = double(z.im.f3);
  return g;
}

static Got words_real(Scalar x) {
  Got g;
  g.nre = 4; g.re[0] = double(x.f0); g.re[1] = double(x.f1);
             g.re[2] = double(x.f2); g.re[3] = double(x.f3);
  g.nim = 1; g.im[0] = 0.0;
  return g;
}
...
//
//     acosh(z) = log(z + sqrt(z*z - 1))          qf_complex.hpp:369-370
//
...
// backend-bounded (src/demo_qf_complex.cpp's is_conditioning_limited list: sub,
...
static const OpTol kOpTols[kOpCount] = {
    ...
    {"add",     28.90},  //  29.00   29.00  1519   bit-exact
    {"sub",     28.90},  //  29.00   29.00  1504   bit-exact
    {"mul",     28.90},  //  29.00   27.77  1375   bit-exact
    {"div",     28.18},  //  28.48   20.71  1275   conditioning-limited
    {"abs",     28.65},  //  28.95   17.62  1059   hypot rounding
    {"conj",    28.90},  //  29.00   29.00  1057   bit-exact
    {"sqrt",    28.64},  //  28.94   16.07  1052   hypot rounding
    {"exp",     27.47},  //  27.77   12.85  1469   zeros of sin/cos near k*pi
    {"log",     28.15},  //  28.45   18.39  1054   conditioning-limited
    {"log10",   28.14},  //  28.44   21.67  1014   conditioning-limited
    {"sin",     27.56},  //  27.86   12.85  1535   zeros of sin/cos near k*pi
    {"cos",     27.47},  //  27.77   12.85  1264   zeros of sin/cos near k*pi
    {"tan",    28.02},  //  28.32   13.46  1486   KI-18 asymptotic branch: ratcheted, was 26.62
    {"asin",    26.40},  //  26.70    0.00  1563   KI-32 (HFT Re asin): ratcheted, was 25.66
    {"acos",    25.55},  //  25.87    0.00  1572   KI-5(c) fixed (Kahan Re + exact Im)
    {"atan",   28.44},  //  28.74   26.71  1540   KI-11/KI-18 component form: ratcheted, was 22.45
    {"sinh",    27.57},  //  27.87   12.85  1539   zeros of sin/cos near k*pi
    {"cosh",    27.48},  //  27.78   12.85  1279   zeros of sin/cos near k*pi
    {"tanh",   27.97},  //  28.27   13.46  1472   KI-18 asymptotic branch: ratcheted, was 26.57
    ...
    {"atanh",  28.36},  //  28.66   15.25  1538   KI-11/KI-18 component form: ratcheted, was 26.77
