// ============================================================================
// tf_complex_accuracy_test.cpp — per-op accuracy gate for TripleFloat COMPLEX
// ============================================================================
//
...
//         tests/tf_complex_accuracy_test.cpp -o /tmp/t && /tmp/t
//
// WHAT IS SCORED
//
// The 24 complex operations shared by all four backends, re-derived from
// include/xp/tf_complex.hpp: the four operators (add sub mul div) plus the 20
...
// convention of src/demo_tf_complex.cpp) as
//
...
// ---------------------------------------------------------------------------
using Scalar  = xp::TripleFloat;
using Complex = xp::TripleFloatComplex;

// 3 x FP32 -> ~21.7 decimal digits. Same cap as tf_accuracy_test / demo_tf_complex.
static constexpr double kMaxDig = 21.7;

static constexpr const char* kBackend = "TF (TripleFloat, 3 x FP32)";

// A corpus input is a `double`. TripleFloat(double) splits it across three FP32 limbs
// (72 bits >= 53), so the split is exact and TF is scored on precisely the value
...
static Got words(Complex z) {
  Got g;
  g.nre = 3; g.re[0] = double(z.re.f0); g.re[1] = double(z.re.f1); g.re[2] = double(z.re.f2);
  g.nim = 3; g.im[0] = double(z.im.f0); g.im[1] = double(z.im.f1); g.im[2] = double(z.im.f2);
  return g;
}

static Got words_real(Scalar x) {
  Got g;
  g.nre = 3; g.re[0] = double(x.f0); g.re[1] = double(x.f1); g.re[2] = double(x.f2);
  g.nim = 1; g.im[0] = 0.0;
  return g;
}
...
//
//     acosh(z) = log(z + sqrt(z*z - 1))          tf_complex.hpp:352
//
...
// backend-bounded (src/demo_tf_complex.cpp's is_conditioning_limited list: sub,
...
static const OpTol kOpTols[kOpCount] = {
    ...
    {"add",     21.60},  //  21.70   21.70  1519   bit-exact
    {"sub",     21.60},  //  21.70   21.70  1504   bit-exact
    {"mul",     21.60},  //  21.70   21.19  1375   bit-exact
    {"div",     21.39},  //  21.69   20.70  1275   conditioning-limited
    {"abs",     21.39},  //  21.69   17.62  1059   hypot rounding
    {"conj",    21.60},  //  21.70   21.70  1057   bit-exact
    {"sqrt",    21.38},  //  21.68   16.07  1052   hypot rounding
    {"exp",     20.30},  //  20.60    6.34  1469   zeros of sin/cos near k*pi
    {"log",     21.06},  //  21.36   14.98  1054   conditioning-limited
    ...
    {"asin",    19.62},  //  19.92    0.00  1563   KI-32 (HFT Re asin): ratcheted, was 18.35
