// ============================================================================
// dd_complex_accuracy_test.cpp — per-op accuracy gate for DoubleDouble COMPLEX
// ============================================================================
//
...
//         tests/dd_complex_accuracy_test.cpp -o /tmp/t && /tmp/t
//
// WHAT IS SCORED
//
// The 24 complex operations shared by all four backends, re-derived from
// include/xp/dd_complex.hpp: the four operators (add sub mul div) plus the 20
...
// convention of src/demo_complex.cpp) as
//
...
// ---------------------------------------------------------------------------
using Scalar  = xp::DoubleDouble;
using Complex = xp::DoubleDoubleComplex;

// 2 x FP64 -> ~31 decimal digits. Same cap as dd_accuracy_test / demo_complex.
static constexpr double kMaxDig = 31.0;

static constexpr const char* kBackend = "DD (DoubleDouble, 2 x FP64)";

// A corpus input is a `double`. DoubleDouble(double) is exact (hi = x, lo = 0),
// so DD is scored on precisely the value the oracle saw. FF is not — see the
// note in ff_complex_accuracy_test.cpp.
...
static Got words(Complex z) {
  Got g;
  g.nre = 2; g.re[0] = z.re.hi; g.re[1] = z.re.lo;
  g.nim = 2; g.im[0] = z.im.hi; g.im[1] = z.im.lo;
  return g;
}

static Got words_real(Scalar x) {
  Got g;
  g.nre = 2; g.re[0] = x.hi; g.re[1] = x.lo;
  g.nim = 1; g.im[0] = 0.0;
  return g;
}
...
//
//     acosh(z) = log(z + sqrt(z*z - 1))          dd_complex.hpp:286-289
//
...
// backend-bounded (src/demo_complex.cpp's is_conditioning_limited list: sub,
...
static const OpTol kOpTols[kOpCount] = {
    ...
    //
    // DD EXCEPTION, worth knowing: the 0.00 minima on `exp`, `sin`, `cos`, `sinh`,
    // `cosh`, `tanh` and `polar` are NOT all conditioning. Some are KI-4 — DD's
    // sincos recovers sin as +/-sqrt(1 - cos^2) and takes the sign from the reduced
    // argument, which flips the SIGN of the result when reduction lands |s3| just
    // above pi (dd_math.hpp:454-498). FF/QF/TF carry both series and are immune.
    // Not excluded: the means pass with margin, so the defect is reported (see
    // docs/KNOWN_ISSUES.md KI-4), not swallowed. Fixing it will RAISE these rows.

    //  op         tol        measured   worst    n     why the gate sits where it does
    {"add",     30.90},  //  31.00   31.00  1519   bit-exact
    {"sub",     30.90},  //  31.00   31.00  1504   bit-exact
    {"mul",     30.90},  //  31.00   31.00  1375   bit-exact
    {"div",     30.90},  //  31.00   31.00  1275   bit-exact
    {"abs",     30.90},  //  31.00   31.00  1059   bit-exact
    {"conj",    30.90},  //  31.00   31.00  1057   bit-exact
    {"sqrt",    30.90},  //  31.00   31.00  1052   bit-exact
    {"exp",     23.27},  //  23.58    0.00  1469   zeros of sin/cos near k*pi
    {"log",     29.61},  //  29.91   16.25  1054   conditioning-limited
    {"log10",   29.74},  //  30.04   16.27  1014   conditioning-limited
    {"sin",     22.81},  //  23.11    0.00  1535   zeros of sin/cos near k*pi
    {"cos",     24.44},  //  24.74    0.00  1264   zeros of sin/cos near k*pi
    {"tan",     22.08},  //  22.38    0.00  1486   conditioning-limited
    ...
    {"atan",    22.33},  //  22.63    0.00  1540   conditioning-limited
    ...
    {"acosh",   28.13},  //  28.43    0.00  1571   KI-1 fixed (Kahan): whole plane, was 957 elems
    {"atanh",   28.84},  //  29.14   15.79  1538   KI-5(b) fixed (complex log1p): ratcheted, was 25.22
    {"pow",     27.75},  //  28.05   15.07  1256   conditioning-limited
    {"polar",   25.41},  //  25.71    0.00  1338   zeros of sin/cos near k*pi
};
// clang-format on
